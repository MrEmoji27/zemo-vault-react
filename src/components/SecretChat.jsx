...
export default function SecretChat({ open, onClose }) {
  ...
  if (minimized) {
    return (
      <div className="secret-dock" onClick={() => { setMinimized(false); setUnread(0); }}>
        <span className="dot" /> Secret Chat {unread > 0 && <span className="badge">{unread}</span>}
      </div>
    );
  }
    ...
    <div className="secret-modal">
      <div className="secret-card">
        <div className="topbar">
          <button className="min" onClick={() => setMinimized(true)}>–</button>
